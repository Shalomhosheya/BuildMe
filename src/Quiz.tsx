...
export default function Quiz() {
  ...
  // Load past attempts to mark quizzes as done
  useEffect(() => {
    quizApi.attempts()
      .then(setAttempts)
  ...
  function startQuiz(quiz: QuizSet) {
    ...
    setServerResult(null); setSubmitError('');
  }
  ...
  async function finishQuiz() {
    ...
    try {
      ...
      // Refresh attempts so the card shows "Completed"
  ...
  function reset() { setActiveQuiz(null); setFinished(false); setServerResult(null); }
    ...
    return (
      <div style={{ padding: '32px 36px', maxWidth: 600 }} className="animate-fadeUp">
        <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 'var(--radius-xl)', padding: 40, textAlign: 'center' }}>
          ...
          {submitting ? (
            <div style={{ fontSize: 13, color: 'var(--text-tertiary)', marginBottom: 20 }}>Saving result...</div>
          ) : (
          ...
          <div style={{ display: 'flex', gap: 10, justifyContent: 'center' }}>
            <button onClick={() => { setQIdx(0); setSelected(null); setAnswered(false); setScore(0); setFinished(false); setServerResult(null); }}
        ...
        <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 'var(--radius-xl)', padding: 28 }}>
          ...
          {answered && (
            <div style={{
              ...
              borderLeft: `3px solid ${selected === q.ans ? 'var(--teal)' : '#E24B4A'}`,
      ...
      <div style={{ display: 'flex', gap: 8, marginBottom: 24, flexWrap: 'wrap' }}>
        {[['all','All levels'],['1','Level 1 — Prestructural'],['2','Level 2 — Unistructural'],['3','Level 3 — Multistructural']].map(([val, label]) => (
          <button key={val} onClick={() => setFilterLevel(val)} style={{
            ...
            fontWeight: filterLevel === val ? 500 : 400, cursor: 'pointer', transition: 'all 0.15s',
          }}>{label}</button>
        ))}
      </div>
        ...
        {filtered.map((quiz, i) => {
          ...
          return (
            <div key={quiz.id} onClick={() => startQuiz(quiz)} style={{
              ...
              transition: 'all 0.15s', animation: `fadeUp 0.4s ease ${i * 0.06}s both`,
            }}
            ...
            >
              <div style={{ display: 'inline-block', background: sc.bg, color: sc.color, fontSize: 11, fontWeight: 600, padding: '3px 9px', borderRadius: 10, marginBottom: 10 }}>
                {quiz.skill}
              </div>
