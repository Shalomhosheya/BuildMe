...
interface PortfolioData {
  user: any; skills: any; stats: any; recentActivity: any;
}

interface CertStatus {
  eligible: boolean; issued: boolean;
  requirements: Record<string, boolean>; readinessPercent: number;
}
...
export default function Portfolio({ onLogout }: { onLogout?: () => void }) {
  ...
  if (error) return (
    <div style={{ padding: '32px 36px', color: '#791F1F', fontSize: 14 }}>
      Error: {error}
    </div>
  );
  ...
  return (
    <div style={{ padding: '32px 36px', maxWidth: 900 }} className="animate-fadeUp">
      ...
      <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 'var(--radius-xl)', padding: 28, display: 'flex', alignItems: 'center', gap: 24, marginBottom: 20 }}>
        ...
        <div style={{ flex: 1 }}>
          ...
          <p style={{ fontSize: 13, color: 'var(--text-secondary)', marginBottom: 12 }}>
            {user.streakDays}-day streak · {skillList.find((s:any) => s.level >= 3)?.levelName ?? 'Prestructural'} learner
          </p>
        ...
        <div style={{ display: 'flex', gap: 24, alignItems: 'center' }}>
          {[
            ...
            [user.estimatedBandScore?.toFixed(1), 'Est. band'],
          ...
          <button onClick={handleLogout} title="Sign out"
            style={{ padding: '8px', border: '1px solid var(--border)', borderRadius: 'var(--radius-md)', background: 'transparent', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 6, fontSize: 12, color: 'var(--text-secondary)', marginLeft: 8 }}
      ...
      <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 'var(--radius-lg)', padding: 20, marginBottom: 20 }}>
        ...
        {!recentActivity.recentQuizzes?.length && !recentActivity.recentEssays?.length && (
      ...
      <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 'var(--radius-lg)', padding: 24 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 20 }}>
          ...
          <button
            onClick={cert?.eligible && !cert?.issued ? handleIssueCert : () => setShowCert(v => !v)}
            disabled={issuing}
            style={{ padding: '9px 20px', background: cert?.eligible ? 'var(--purple)' : 'var(--gray-400)', color: '#fff', border: 'none', borderRadius: 'var(--radius-md)', fontSize: 13, fontWeight: 500, cursor: cert?.eligible ? 'pointer' : 'not-allowed' }}
          >
            {issuing ? 'Issuing...' : cert?.issued ? (showCert ? 'Hide certificate' : 'View certificate') : cert?.eligible ? 'Issue certificate' : 'Preview'}
          </button>
        </div>
        ...
        {showCert && (
          <div style={{ marginTop: 20, animation: 'fadeUp 0.4s ease both' }}>
            <div style={{ border: '2px solid var(--purple)', borderRadius: 'var(--radius-xl)', padding: 40, textAlign: 'center', position: 'relative', overflow: 'hidden' }}>
              ...
              <div style={{ fontFamily: 'var(--font-display)', fontSize: 32, color: 'var(--purple)', marginBottom: 20 }}>{certData?.userName ?? user.name}</div>
              ...
              <div style={{ display: 'flex', justifyContent: 'center', gap: 40, marginBottom: 24 }}>
                {[
                  [certData?.estimatedBand?.toFixed(1) ?? user.estimatedBandScore?.toFixed(1), 'Estimated band'],
                  [certData?.totalPoints ?? user.totalPoints, 'Points earned'],
                  [certData?.quizzesCompleted ?? stats.quizzesCompleted, 'Quizzes completed'],
                ].map(([val, lbl]) => (
                  <div key={lbl as string} style={{ textAlign: 'center' }}>
              ...
              <div style={{ borderTop: '1px solid var(--border)', paddingTop: 16, display: 'flex', justifyContent: 'space-between', fontSize: 11, color: 'var(--text-tertiary)' }}>
                <span>Issue date: {new Date().toLocaleDateString('en-GB', { month: 'long', year: 'numeric' })}</span>
                <span>Cert ID: {certData?.certId ?? 'BM-PREVIEW'}</span>
                <span>{certData ? `buildme.app/verify/${certData.certId}` : 'Complete all requirements to unlock'}</span>
              </div>
            </div>
          </div>
        )}
